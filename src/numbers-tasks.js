...
function toNumber(value, def) {
  ...
  if (Number.isFinite(convertValue)) {
    return convertValue;
  }
  return def;
}
...
function getRandomInteger(min, max) {
  let minСeil = min;
  let maxFloor = max;
  minСeil = Math.ceil(min);
  maxFloor = Math.floor(max);
  return Math.floor(Math.random() * (maxFloor - minСeil + 1)) + minСeil;
}
/**
...
function getCountOfOddNumbers(number) {
  const onlyPositive = Math.abs(number);
  if (onlyPositive % 2 === 0) {
    return onlyPositive / 2;
  }
  return Math.ceil(onlyPositive / 2);
}
